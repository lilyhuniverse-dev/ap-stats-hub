...
function initCorrelation() {
  setOptions($('corrSheetSelect'), Object.keys(sheets), Object.keys(sheets)[0], displaySheetName);
  ['corrSheetSelect','corrXSelect','corrYSelect'].forEach(id => $(id).disabled = false);
  loadCorrelationSheet($('corrSheetSelect').value);
}
function loadCorrelationSheet(name) {
  const cols = numericColumns(sheets[name] || []);
  setOptions($('corrXSelect'), cols, cols[0], displayVariableName); setOptions($('corrYSelect'), cols, cols[1] || cols[0], displayVariableName);
  corrX = $('corrXSelect').value; corrY = $('corrYSelect').value; resetCorrelation();
}
function resetCorrelation() {
  const rows = sheets[$('corrSheetSelect').value] || [];
  corrOriginal = rows.map(r => ({x:Number(r[corrX]), y:Number(r[corrY])})).filter(p => Number.isFinite(p.x) && Number.isFinite(p.y));
  corrEdited = corrOriginal.map(p => ({...p}));
  $('pointSlider').max = Math.max(0, corrEdited.length - 1); $('pointSlider').value = 0; syncCorrelationSliders(); renderCorrelation();
}
function syncCorrelationSliders() {
  const p = corrEdited[Number($('pointSlider').value)]; if (!p) return;
  const xd = domain(corrOriginal.map(q=>q.x)), yd = domain(corrOriginal.map(q=>q.y));
  [['corrXSlider','corrXValue',p.x,xd],['corrYSlider','corrYValue',p.y,yd]].forEach(([id,out,value,d]) => { const s=$(id); s.min=d[0]; s.max=d[1]; s.step=(d[1]-d[0])/200; s.value=value; $(out).textContent=fmt(value); });
  $('pointValue').textContent = Number($('pointSlider').value) + 1;
}
function correlationStats(points) {
  if (!points.length) return {mx:0,my:0,vx:0,vy:0}; const n=points.length, mx=points.reduce((s,p)=>s+p.x,0)/n, my=points.reduce((s,p)=>s+p.y,0)/n;
  return {mx,my,vx:points.reduce((s,p)=>s+(p.x-mx)**2,0)/n,vy:points.reduce((s,p)=>s+(p.y-my)**2,0)/n};
}
function renderSmallChart(svg, points, xd, yd, changed) {
  if (!svg || !points.length) return; const w=520,h=390,m={l:55,r:18,t:16,b:54}, sx=v=>scale(v,xd,m.l,w-m.r),sy=v=>scale(v,yd,h-m.b,m.t); let html='';
  ticks(xd[0],xd[1],5).forEach(v=>{const x=sx(v);html+=`<line class="grid" x1="${x}" y1="${m.t}" x2="${x}" y2="${h-m.b}"/><text class="tick" x="${x}" y="${h-m.b+19}" text-anchor="middle">${fmt(v)}</text>`});
  ticks(yd[0],yd[1],5).forEach(v=>{const y=sy(v);html+=`<line class="grid" x1="${m.l}" y1="${y}" x2="${w-m.r}" y2="${y}"/><text class="tick" x="${m.l-8}" y="${y+4}" text-anchor="end">${fmt(v)}</text>`});
  html+=`<line class="axis" x1="${m.l}" y1="${h-m.b}" x2="${w-m.r}" y2="${h-m.b}"/><line class="axis" x1="${m.l}" y1="${m.t}" x2="${m.l}" y2="${h-m.b}"/>`;
  points.forEach((p,i)=>html+=`<circle class="point${changed&&i===Number($('pointSlider').value)?' selected':''}" cx="${sx(p.x)}" cy="${sy(p.y)}" r="5"/>`);
  html+=`<text class="axis-label small-axis" x="${w/2}" y="${h-10}" text-anchor="middle">${escapeHtml(displayVariableName(corrX))}</text><text class="axis-label small-axis" transform="translate(15 ${h/2}) rotate(-90)" text-anchor="middle">${escapeHtml(displayVariableName(corrY))}</text>`; svg.innerHTML=html;
}
function updateCorrelationLanguage() {
  if (!$('corrTitle')) return; const t=translations[lang];
  [['corrTitle','corr_title'],['corrDescription','corr_desc'],['corrSheetLabel','label_sheet'],['corrXLabel','label_x'],['corrYLabel','label_y'],['corrResetButton','corr_reset'],['sliderHeading','corr_slider_heading'],['pointLabel','corr_point'],['corrXValueLabel','corr_x_value'],['corrYValueLabel','corr_y_value'],['corrHint','corr_hint'],['originalHeading','corr_original'],['changedHeading','corr_changed']].forEach(([id,k])=>$(id).textContent=t[k]);
  if (Object.keys(sheets).length) { setOptions($('corrSheetSelect'),Object.keys(sheets),$('corrSheetSelect').value,displaySheetName); const cols=numericColumns(sheets[$('corrSheetSelect').value]||[]); setOptions($('corrXSelect'),cols,corrX,displayVariableName); setOptions($('corrYSelect'),cols,corrY,displayVariableName); }
}
function renderCorrelation() {
  if (!corrOriginal.length || !$('originalChart')) return; const t=translations[lang], all=[...corrOriginal,...corrEdited], xd=domain(all.map(p=>p.x)), yd=domain(all.map(p=>p.y)), a=correlationStats(corrOriginal),b=correlationStats(corrEdited);
  const stat=s=>`${t.mean} X: ${fmt(s.mx)}　${t.variance} X: ${fmt(s.vx)}<br>${t.mean} Y: ${fmt(s.my)}　${t.variance} Y: ${fmt(s.vy)}`;
  $('originalStats').innerHTML=stat(a); $('changedStats').innerHTML=stat(b); renderSmallChart($('originalChart'),corrOriginal,xd,yd,false); renderSmallChart($('changedChart'),corrEdited,xd,yd,true);
}
$('scatterNav').addEventListener('click',()=>{ $('scatterTool').classList.remove('tool-hidden');$('correlationTool').classList.add('tool-hidden');$('scatterNav').classList.add('active');$('correlationNav').classList.remove('active'); });
$('correlationNav').addEventListener('click',()=>{ $('scatterTool').classList.add('tool-hidden');$('correlationTool').classList.remove('tool-hidden');$('scatterNav').classList.remove('active');$('correlationNav').classList.add('active');renderCorrelation(); });
$('corrSheetSelect').addEventListener('change',e=>loadCorrelationSheet(e.target.value));
$('corrXSelect').addEventListener('change',()=>{corrX=$('corrXSelect').value;resetCorrelation();}); $('corrYSelect').addEventListener('change',()=>{corrY=$('corrYSelect').value;resetCorrelation();});
$('pointSlider').addEventListener('input',()=>{syncCorrelationSliders();renderCorrelation();});
$('corrXSlider').addEventListener('input',e=>{const p=corrEdited[Number($('pointSlider').value)];if(p){p.x=Number(e.target.value);$('corrXValue').textContent=fmt(p.x);renderCorrelation();}});
$('corrYSlider').addEventListener('input',e=>{const p=corrEdited[Number($('pointSlider').value)];if(p){p.y=Number(e.target.value);$('corrYValue').textContent=fmt(p.y);renderCorrelation();}});
$('corrResetButton').addEventListener('click',resetCorrelation);
